...
from framework.backend import BaseBackend
from framework.transform import GraphMutator
...
class BaseOptimizer:
    ...
    def _profile(
        ...
        if self._always_simulation:
            ...
            return RunResult(False, latency, sim_res.peak_memory)
        ...
        try:
            ...
            measured_latency = self._backend.measure_graph_latency(number=number, repeat=repeat)
            final_latency = measured_latency * reduce if flag else measured_latency
            final_mem = self._backend.measure_graph_peak_memory() * ty if flag else self._backend.measure_graph_peak_memory()
            return RunResult(
                True,
                final_latency,
                final_mem,
...
class RelaxOptimizer(BaseOptimizer):
    def __init__(
        ...
        relax_ratio=1.1,
        early_stop=None,
        stop_when_mem_met=False,** kwargs,
    ) -> None:
        super().__init__(
            scheduler,
            backend,
